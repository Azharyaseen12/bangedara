  ...
  return (
    <ul className="divide-y divide-gray-200">
      {comments.map(comment => {
        const commentDirection = detectLanguage(comment.content);
            ...
            <div className="flex gap-3 items-start">
              ...
              <div className="flex-1">
                ...
                <div 
                  ...
                  style={{ textAlign: commentDirection === 'rtl' ? 'right' : 'left' }}
                >
                  {comment.content}
                </div>
                {canReply && (
                  ...
                  >
                    <div className="flex items-center gap-2">
                      <input
                        ...
                        dir={detectLanguage(replyContent[comment.id] || '')}
                        style={{ textAlign: detectLanguage(replyContent[comment.id] || '') === 'rtl' ? 'right' : 'left' }}
                      />
                ...
                {comment.replies && comment.replies.length > 0 && (
                  <ul className="mt-2 space-y-2 pl-6 border-l-2 border-emerald-50">
                    {comment.replies.map((reply) => {
                      const replyDirection = detectLanguage(reply.content);
                          ...
                          <div className="flex gap-2 items-start">
                            ...
                            <div className="flex-1">
                              ...
                              <div 
                                ...
                                dir={replyDirection}
                                style={{ textAlign: replyDirection === 'rtl' ? 'right' : 'left' }}
                              >
                                {reply.content}
                              </div>
                            </div>
